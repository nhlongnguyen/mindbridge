...
from mindbridge.jobs.celery_config import CeleryConfig, CeleryBrokerConfig
from mindbridge.jobs.celery_app import create_celery_app, get_celery_app
from mindbridge.jobs.exceptions import BrokerConnectionError, BrokerConfigurationError


class TestCeleryBrokerConfig:
    """Test cases for Celery broker configuration."""

    def test_celery_broker_config_creation_success(self):
    ...
    def test_celery_broker_config_default_values(self):
    ...
    def test_celery_broker_config_invalid_broker_url(self):
        ...
        with pytest.raises(ValueError, match="Broker URL cannot be empty"):
            CeleryBrokerConfig(broker_url=None)

    def test_celery_broker_config_invalid_serializer(self):
        """Failure case: Invalid serializer."""
        with pytest.raises(ValueError, match="Invalid task serializer"):
            CeleryBrokerConfig(
                broker_url="redis://localhost:6379/0",
                task_serializer="invalid"
            )

        with pytest.raises(ValueError, match="Invalid result serializer"):
            CeleryBrokerConfig(
                broker_url="redis://localhost:6379/0",
                result_serializer="invalid"
            )

    def test_celery_broker_config_invalid_content_types(self):
        ...
        with pytest.raises(ValueError, match="Invalid content type"):
            CeleryBrokerConfig(
                broker_url="redis://localhost:6379/0",
                accept_content=["invalid"]
            )

    def test_celery_broker_config_invalid_visibility_timeout(self):
    ...
    def test_celery_broker_config_redis_url_parsing(self):
        """Expected use case: Parse Redis URLs correctly."""
        config = CeleryBrokerConfig(
            broker_url="redis://user:pass@localhost:6380/2"
        )
    ...
    def test_celery_broker_config_to_dict(self):
...
class TestCeleryConfig:
    ...
    @pytest.fixture
    def celery_config(self, broker_config):
    ...
    def test_celery_config_creation_success(self, broker_config):
    ...
    def test_celery_config_default_values(self, broker_config):
    ...
    def test_celery_config_invalid_broker_config(self):
        """Failure case: Invalid broker config."""
        with pytest.raises(TypeError):
            CeleryConfig(broker_config=None)

        with pytest.raises(TypeError):
            CeleryConfig(broker_config="invalid")

    def test_celery_config_invalid_task_routes(self, broker_config):
    ...
    def test_celery_config_invalid_beat_schedule(self, broker_config):
        """Failure case: Invalid beat schedule."""
        with pytest.raises(TypeError):
            CeleryConfig(
                broker_config=broker_config,
                beat_schedule="invalid"
            )

    def test_celery_config_invalid_worker_settings(self, broker_config):
    ...
    def test_celery_config_time_limit_validation(self, broker_config):
        """Failure case: Time limit validation."""
        with pytest.raises(ValueError, match="Task time limit must be greater than soft time limit"):
            CeleryConfig(
                broker_config=broker_config,
    ...
    def test_celery_config_get_celery_settings(self, celery_config):
...
class TestCeleryApp:
    ...
    def test_create_celery_app_success(self, celery_config):
    ...
    def test_create_celery_app_with_custom_name(self, celery_config):
    ...
    def test_create_celery_app_invalid_config(self):
        """Failure case: Create Celery app with invalid config."""
        with pytest.raises(TypeError):
            create_celery_app(None)

        with pytest.raises(TypeError):
            create_celery_app("invalid")

    def test_get_celery_app_success(self, celery_config):
    ...
    def test_get_celery_app_configuration_error(self):
        ...
        with patch('mindbridge.jobs.celery_app.get_celery_config') as mock_get_config:
            ...
            with pytest.raises(BrokerConfigurationError, match="Failed to configure Celery"):
                get_celery_app()

    @pytest.mark.asyncio
    async def test_celery_broker_connection_check_success(self, celery_config):
    ...
    @pytest.mark.asyncio
    async def test_celery_broker_connection_check_failure(self, celery_config):
        """Failure case: Broker connection check fails."""
        with patch('mindbridge.jobs.celery_app.create_celery_app') as mock_create:
            mock_app = MagicMock()
            mock_app.control.inspect.return_value.ping.side_effect = Exception("Connection error")
            mock_create.return_value = mock_app
    ...
    @pytest.mark.asyncio
    async def test_celery_broker_connection_check_no_workers(self, celery_config):
    ...
    def test_celery_app_task_registration(self, celery_config):
    ...
    def test_celery_app_autodiscover_tasks(self, celery_config):
        """Expected use case: Auto-discover tasks."""
        with patch('mindbridge.jobs.celery_app.Celery') as mock_celery_class:
            mock_app = MagicMock()
            mock_celery_class.return_value = mock_app
            
            app = create_celery_app(celery_config, autodiscover_tasks=True)
            
            mock_app.autodiscover_tasks.assert_called_once_with([
                "mindbridge.jobs.tasks",
            ])

    def test_celery_config_environment_variables(self):
    ...
    def test_celery_config_environment_variables_missing(self):
        """Failure case: Missing required environment variables."""
        with patch.dict('os.environ', {}, clear=True):
            from mindbridge.jobs.celery_config import get_celery_config_from_env
            
            with pytest.raises(BrokerConfigurationError, match="REDIS_URL environment variable is required"):
                get_celery_config_from_env()

    def test_celery_config_environment_variables_invalid(self):
        ...
        with patch.dict('os.environ', {
            ...
            with pytest.raises(BrokerConfigurationError, match="Invalid CELERY_VISIBILITY_TIMEOUT"):
                get_celery_config_from_env()

    def test_celery_broker_health_check_success(self, celery_config):
    ...
    def test_celery_broker_health_check_failure(self, celery_config):
        """Failure case: Broker health check fails."""
        with patch('mindbridge.jobs.celery_app.create_celery_app') as mock_create:
            mock_app = MagicMock()
            mock_app.control.inspect.return_value.stats.side_effect = Exception("Connection error")
            mock_create.return_value = mock_app
    ...
    def test_celery_task_routing_configuration(self):
        ...
        assert task_routes["mindbridge.jobs.tasks.process_document"]["queue"] == "document_processing"
    ...
    def test_celery_beat_schedule_configuration(self):
        ...
        assert beat_schedule["cleanup-expired-cache"]["task"] == "mindbridge.jobs.tasks.cleanup_expired_cache"
        assert beat_schedule["cleanup-expired-cache"]["schedule"] == 3600.0

    def test_celery_worker_configuration(self, celery_config):
    ...
    def test_celery_transport_options_configuration(self, celery_config):
