...
def get_celery_app() -> Celery:
    ...
    if _celery_app is None:
        try:
            ...
            raise BrokerConfigurationError(f"Failed to configure Celery: {e}", e)
...
def get_queue_lengths(config: CeleryConfig) -> Dict[str, int]:
    ...
    try:
        ...
        def _purge_queue():
            try:
                ...
                raise BrokerConnectionError(f"Failed to purge queue: {e}", e)
        
        result = await loop.run_in_executor(None, _purge_queue)
        return result or 0
        
    except Exception as e:
        raise BrokerConnectionError(f"Failed to purge queue {queue_name}: {e}", e)


async def shutdown_workers(config: CeleryConfig, signal: str = "TERM") -> bool:
    ...
    try:
        ...
        def _shutdown_workers():
            try:
                ...
                raise BrokerConnectionError(f"Failed to shutdown workers: {e}", e)
